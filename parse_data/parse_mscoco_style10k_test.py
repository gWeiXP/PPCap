...
if __name__ == "__main__":
    ...
    if change_data_num:
        data_mscoco_filenames_saved = []
        data_mscoco_numchanged_train = []
        for i in range(len(data_mscoco_removed_train)):
            if not data_mscoco_removed_train[i]["filename"] in data_mscoco_filenames_saved:
                data_mscoco_numchanged_train.append(data_mscoco_removed_train[i])
                data_mscoco_filenames_saved.append(data_mscoco_removed_train[i]["filename"])
        data_mscoco_removed_train = data_mscoco_numchanged_train
    ...
    if change_data_num:
        data_mscoco_style10k_train_path = "/home/liwc/wxp/refercode/GeDi_Final/data/dataset_train/MSCOCO_style10k_test_ViT-L_14_train.pkl"
